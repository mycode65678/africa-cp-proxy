...
import {
  UserLists,
  OrderDel, ReleaseFunds, userUpdate, LeaveList, StatusChange, OrderLists,
} from '@/services/api';
const TableList: React.FC = () => {
  ...
  const columns = [
    ...
    {
      title: intl.formatMessage({id: 'LotteryName',}),
      dataIndex: 'lotteryName',
      search: false,
      render: (_, record) => {
        return <span>{record.Lottery?.Name}</span>
      }
    },
    {
    ...
    {
      title: intl.formatMessage({id: 'Multiple',}),
      dataIndex: 'BettingNumber',
      search: false,
    },
    {
      title: intl.formatMessage({id: 'Details',}),
      dataIndex: 'BettingContent',
      search: false,
      renderText: (_, record) => {
        return intl.formatMessage({id: record.BettingContent,})
      }
    },
    {
    ...
    },
    {
      ...
      search: false,
      render: (_, record) => {
        return record.OddsSetting?.Odds
      }
    },
    {
    ...
    // },
    {
      title: intl.formatMessage({id: 'LotteryResult',}),
    ...
    },
    {
      ...
      render: (_, record) => {
      //   正数蓝色，负数红色
        return <span style={{color: Number(record.ProfitLoss) > 0 ? 'blue' : 'red'}}>{record.ProfitLoss}</span>
      }
    },
    {
      ...
      valueEnum: {
        1: {
          text: intl.formatMessage({id: 'Redeemed',}),
          status: 'Success',
        },
        2: {
          text: intl.formatMessage({id: 'WaitingForRedemption',}),
          status: 'Error',
        },
        3: {
